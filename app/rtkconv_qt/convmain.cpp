...
void MainWindow::LoadOpt(void)
{
    ...
    RnxVer              =ini.value ("opt/rnxver",      0).toInt();		// 6?
    ...
    NavSys              =ini.value ("opt/navsys",    0x3).toInt();	// SYS_ALL
    ObsType             =ini.value ("opt/obstype",   0xF).toInt();
    FreqType            =ini.value ("opt/freqtype",  0x3).toInt();	// FREQTYPE_L1|FREQTYPE_L2|FREQTYPE_L7
    ...
    ScanObs             =ini.value ("opt/scanobs",     0).toInt();	// 1
